...
MarshMallow is mainly a simple marshalling model that can be used to serialize and de-serialize HTTP requests/responses. (JSON only as of now). Also, it provides a wrapper at the top of Python's [requests](http://docs.python-requests.org/en/master/) library, integrated with the MarshMallow base models. 

**MarshMallow** is created mainly for performing automated testing of REST APIs.
...
MarshMallow requires Python 3.X or more. However, this should work with python 2.7.x as well, but its not tested.
